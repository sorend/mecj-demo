...
public class ProteinSequence {
	...
	public static ProteinSequence[] load(String resourcePath) throws IOException {
		InputStream in = ProteinSequence.class.getResourceAsStream(resourcePath);
	...
	public static ProteinSequence[] load(Reader reader) throws IOException {
		...
		while (true) {
			...
			if (line == null)
				break;
			String[] a = line.split(",");
			list.add(new ProteinSequence(stringToDoubles(a[0]), Integer.parseInt(a[1])));
		}
		br.close();
